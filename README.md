...
| `merged` | 检查已合并到 master | `feops merged feature/auth` |
| `into` | 检查 B 是否已合并到 A | `feops into release feature/auth` |
...
feops branch feature/new-ui
```

### 场景 3: 功能分支管理
```bash
# 1. 查找包含该分支的项目
feops branch feature/auth
...
# 2. 检查功能分支是否已合并到 release
feops into release feature/payment
...
│   │   ├── list.ts         # 仓库列表命令
│   │   ├── branch.ts       # 分支查找命令
│   │   ├── merged.ts       # 合并检查命令（合并到 master）
│   │   ├── into.ts         # 合并检查命令（合并到任意分支）
│   │   └── uptodate.ts     # 最新代码检查命令
│   └── utils/              # 工具函数
│       ├── index.ts
│       ├── directories.ts  # 多 Group 目录解析
│       ├── gitRef.ts       # 分支 ref 解析
│       ├── mergeCheck.ts   # 合并状态检查
│       ├── markdownReport.ts  # Markdown 报告生成
│       └── progressBar.ts
├── dist/                   # 编译输出
├── docs/                   # 文档
│   ├── commands.md         # 命令详细参考
...
```bash
feops branch <name> [-d <dir>] [-g <group>] [--local] [--no-fetch] [--format <type>] [-p <n>] [-o <file>]
```

默认检查 `origin/*` 远程跟踪分支；加 `--local` 恢复本地优先行为。`--remote` 已弃用（等同于默认）。

### merged - 检查合并状态

```bash
feops merged <branch> [-d <dir>] [-g <group>] [--base-branch <branch>] [--local] [--show-missing] [--no-fetch] [--format <type>] [-p <n>] [-o <file>] [--merge-mode <mode>]
```

等价于 `feops into <base-branch> <branch>`，默认目标分支为 `master`。

### into - 检查 B 是否已合并到 A

```bash
feops into <target> <source> [-d <dir>] [-g <group>] [--local] [--show-missing] [--no-fetch] [--format <type>] [-p <n>] [-o <file>] [--merge-mode <mode>]
```

检查 `source`（B）是否已合并到 `target`（A）。默认基于 `origin/*` 远程跟踪分支；加 `--local` 改为本地优先。

示例：

- `feops into release feature/payment` — 检查 `feature/payment` 是否已合并到 `release`
- `feops into feature-attpay-v1 master` — 检查 **master 是否已合入 feature-attpay-v1**（与 `feops uptodate feature-attpay-v1 --base-branch master` 同向）

`--merge-mode` 控制检测方式（默认 `auto`）：

| 模式 | 说明 |
|------|------|
| `strict` | 仅 commit 祖先检测（`merge-base --is-ancestor`） |
| `content` | 仅补丁等价检测（`git cherry`，支持 squash/cherry-pick） |
| `auto` | 先 strict，失败再 content |

导出 Markdown 报告：`feops into release feature/payment -o report.md`

### uptodate - 检查是否最新

```bash
feops uptodate <branch> [-d <dir>] [-g <group>] [--base-branch <branch>] [--local] [--show-missing] [--no-fetch] [--format <type>] [-p <n>] [-o <file>]
```
...
### merged vs into vs uptodate 的区别

- `into A B`：B → A 是否已合并（B 是否已在 A 的历史/补丁中）
- `merged B --base-branch A`：与 `into A B` 等价
- `merged B`：与 `into master B` 等价
- `uptodate B --base-branch A`：B 是否包含 A 最新代码（与 `into B A` 同向，但 uptodate 会显示落后提交数）
- 三者默认均基于 `origin/*` 远程跟踪分支比较；加 `--local` 可改为本地优先

### 排查「已 merge 但显示未合并」

以 `feops into feature-attpay-v1 master` 为例，常见原因：

1. **master 又有新提交** — 检查的是 master 当前 tip，merge 之后 master 前进会判为未合并
2. **origin ref 与本地不一致** — 默认查 `origin/*`，可用 `--local` 或先 `git fetch`
3. **Squash merge** — strict 检测会失败，使用默认 `--merge-mode auto` 或 `content`

手动验证（在目标仓库内）：

```bash
git fetch --all --prune
git merge-base --is-ancestor origin/master origin/feature-attpay-v1  # strict
git rev-list --count origin/feature-attpay-v1..origin/master         # 落后提交数
git cherry -v origin/feature-attpay-v1 origin/master                 # 补丁等价
```

### 输出格式

大部分命令支持三种终端输出格式：`table`（默认）、`simple`、`json`。

`branch` / `merged` / `into` / `uptodate` 另支持 `-o, --output <file>`，在终端输出之外额外写入 Markdown 报告文件。
